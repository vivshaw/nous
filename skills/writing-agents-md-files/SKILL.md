...
Read `writing-agent-directives` first, for the underlying guidance on token efficiency, compliance techniques, and directive structure.
...
Treat `AGENTS.md` as canonical. Preserve host-specific companion files only when the repository already uses them or the user explicitly requests them; do not duplicate the instructions.
...
| Boundaries | What agents can and cannot edit |
...
**Avoid host-specific import syntax** such as `@./service.ts`; it may force-load files and is not portable. Name files normally so an agent can read them when needed.
...
| Missing freshness date | Always include; obtain the real date from the environment |
| Using host-specific imports for files | Name files normally and let the agent read them on demand |
