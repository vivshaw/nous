...
- `explore-researching-on-the-internet` for crate documentation, API behavior, and ecosystem conventions.
- Ask that research subagent to inspect remote source when documentation does not reveal the needed patterns or reference implementation.
...
- **Verify latest versions before adding or bumping.** Use `explore-researching-on-the-internet` to look up the current version on crates.io. Memorized versions go stale quickly.
