#!/usr/bin/env python3
"""Check a chart palette against the measurable rules in `visualizing-data`.
...
def simulate(value: str, kind: str) -> tuple[float, float, float]:
    ...
    return tuple(  # type: ignore[return-value]
        min(1.0, max(0.0, sum(row[i] * rgb[i] for i in range(3)))) for row in CVD_MATRIX[kind]
    )
...
def check_categorical(palette: list[str], mode: str, surface: str, pairs: str) -> list[Check]:
    ...
    checks.append(
        (
            "Lightness band",
            "pass" if not off_band else "fail",
            f"all {len(palette)} within L {low}-{high}"
            if not off_band
    ...
    checks.append(
        (
            "Chroma floor",
            "pass" if not grey else "fail",
            f"all {len(palette)} at or above C {CHROMA_FLOOR}"
            if not grey
        ...
        checks.append(
            (
                ...
                + (
                    ""
                    if passed
                    else f" - under {PLAIN_SIGHT_FLOOR:.0f}, so full-colour readers struggle too"
                ),
            )
        )
    ...
    checks.append(
        (
            "Surface contrast",
            "pass" if not dim else "warn",
            f"all {len(palette)} at or above {CONTRAST_FLOOR:g}:1"
            if not dim
            else (
                f"under {CONTRAST_FLOOR:g}:1, needs visible labels or a table view: "
                f"{fmt_pairs(dim)}"
            ),
        )
    )
...
def check_ramp(palette: list[str], mode: str, surface: str, ordinal: bool) -> list[Check]:
    ...
    *supposed* to span the lightness band and to place its steps close together.
    ...
    crowded = [
        (palette[i], palette[i + 1], round(abs(lightness[i + 1] - lightness[i]), 3))
    ...
    checks.append(
        (
            "Step separation",
            "pass" if not crowded else "fail",
            f"every gap at or above dL {RAMP_MIN_STEP}"
            if not crowded
    ...
    pale = (
        max(palette, key=lambda c: oklch(c)[0])
        if mode == "light"
    ...
    if ratio >= RAMP_PALE_END_FLOOR:
        ...
        pale_state = "warn"
        note = (
            f" - under {RAMP_PALE_END_FLOOR:g}:1, so this step reads as empty surface; "
    ...
    checks.append(
        (
            "Hue stability",
            "pass" if spread <= RAMP_HUE_DRIFT else "fail",
...
def main() -> int:
    ...
    parser.add_argument(
        "--surface", default=None, help="chart surface hex; defaults to the mode's surface"
    )
    parser.add_argument(
        "--pairs",
        choices=("adjacent", "all"),
        default="adjacent",
        help="adjacent for stacks/bars/lines (default); all for scatter/bubble/map/small-multiples",
    )
    parser.add_argument(
        "--ramp",
        action="store_true",
    ...
    parser.add_argument(
        "--ordinal", action="store_true", help="ramp mode for discrete ordered marks"
    )
    ...
    if warned:
        ...
        print(
            "  A warning is legal only with its mitigation: a second encoding "
