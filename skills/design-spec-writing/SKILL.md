...
Copy the adjacent `spec-template.md` to `.gro/tasks/YYYY-MM-DD-{slug}/spec.md` and fill it in. The template carries per-section guidance in HTML comments — delete every one of them as you go, including the header comment. A spec that ships with its scaffolding still attached reads as unfinished.
...
(1) Copy this:
    ...
    Use the project-getting-started skill for .gro/tasks/{slug}/spec.md
