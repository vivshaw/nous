...
Use `howto-code-in-typescript` for general TypeScript patterns. This skill covers React-specific usage only.
...
4. **Need external library?** -> Use `explore-investigating-a-codebase` to find existing patterns, or `explore-researching-on-the-internet` to evaluate options (Zustand, Redux Toolkit, TanStack Query)
...
Use `explore-investigating-a-codebase` first to find existing test patterns. Common approaches: React Testing Library, Playwright, Cypress.
