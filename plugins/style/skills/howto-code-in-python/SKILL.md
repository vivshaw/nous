...
See `style:writing-comments` for the general rules. Python-specific:
...
- Validate at the boundary — deserialization, request handling, CLI parsing — and let the interior trust its types. See `style:defense-in-depth`.
...
See `style:writing-good-tests` for what to test, and `style:property-based-testing` (Hypothesis, in Python) for generative cases. Python-specific:
